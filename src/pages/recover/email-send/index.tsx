export default function EmailSend() {
  return (
    <div
      className="flex w-full h-screen bg-no-repeat bg-center bg-cover justify-between items-center "
      style={{ backgroundImage: "url(../src/assets/backgroundDesktop.png)" }}
    >
    <div className="w-[60%] ml-4 lg:block hidden h-[95%] transition-all">
        <img
          src="..\src\assets\er.png"
          alt="imagem-decorativa"
          className=" w-full h-full lg:block hidden"
        />
      </div>
      <div className="flex w-full lg:w-[25%] h-full flex-col items-center px-10 lg:px-0justify-center gap-10 lg:mr-20">
      <div className="">
        <img src="..\src\assets\logoHorizon.png" alt="logo" />
      </div>
      <div className="">
        <img src="..\src\assets\folders.png" alt="logo" />
      </div>
      <div className="w-full flex flex-col items-center">
        <p className="text-[#D9D9D9] text-[20px] font-semibold mb-4">
          E-mail enviado!
        </p>
        <p className="text-[#D9D9D9] w-[70%] text-center text-[14px]">
          Verifique sua <span className="text-[#136D52] font-semibold semibold italic">caixa de mensagens</span> ou <span className="text-[#136D52] font-semibold italic">spam</span>, para redefinir sua senha.
        </p>
      </div>
     
      <div className="w-full items-center flex flex-col gap-4">
        <button onClick={()=> window.location.href = "/login"} className="bg-[linear-gradient(90deg,#139C73,#136D52)] w-full py-3 rounded-full text-white font-bold">
          VOLTAR PARA O LOGIN
        </button>
      </div>
    </div>
    </div>
  );
}
